    ...
    content_tag(:div, class: "card mx-2 p-0 shadow-sm my-1 card-main-model-section") do
      concat(content_tag(:div, class: "card-header rounded bg-dark text-light py-1 py-md-2 px-2 px-md-3 d-flex align-items-center") do
        concat(content_tag(:div, class: "card-main-model-header-row d-flex flex-nowrap flex-md-wrap align-items-center gap-1 gap-md-2 w-100 min-w-0") do
          concat(content_tag(:div, class: "d-flex align-items-center min-w-0 flex-shrink-1 gap-2 gap-md-3") do
            concat(content_tag(:i, nil, class: "bi bi-xl brand-colored bi-#{icon} flex-shrink-0 card-main-model-header-icon"))
